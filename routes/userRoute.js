    ...
    connection.query(query, [user.email], (err, results) => {
        if (!err) {
            if (results.length <= 0) {
                ...
                connection.query("insert into users(name, contact, email, password, status, role) value(?,?,?,?, 'false', 'user')",
                [user.name, user.contact, user.email, user.password], (err, results) => {
                    if (!err) {
